...
var Toolkit = {
   ...
   */
  hookMethodReturn(className, methodName, forceReturn) {
    try {
   ...
   */
  blockMethod(className, methodName) {
    try {
      const klass = this.findClass(className);
      if (!klass) {
        console.log(`[-] Error: Class '${className}' not found in any assembly.`);
  ...
  Unity: {
    forceClick(targetName) {
      try {
        ...
        const go = findMethod.invoke(Il2Cpp.string(targetName));
        if (!go.isNull()) {
          sendMsgMethod.invoke(go, Il2Cpp.string("OnClick"));
          console.log(`[\u{1F680}] SUCCESS: Force clicked GameObject "${targetName}"`);
        } else {
    ...
    trackStatus(targetObjName) {
      try {
        const GameObject = Il2Cpp.domain.assembly("UnityEngine.CoreModule").image.class("UnityEngine.GameObject");
        const setActive = GameObject.method("SetActive");
        const getName = GameObject.method("get_name");
        setActive.implementation = function(state) {
          const name = getName.invoke(this).content;
          if (name && name.toLowerCase().includes(targetObjName.toLowerCase())) {
            console.log(`
[\u{1F3AF} TARGET DETECTED] GameObject: ${name} | State: ${state}`);
            if (state === 0 || state === false) {
              console.log(`[!] Forcing Active (True)!`);
              return setActive.invoke(this, 1);
            }
          }
          return setActive.invoke(this, state);
        };
    ...
    replaceUIText(searchString, replaceString) {
      try {
                ...
                method.implementation = function(text) {
                  if (text instanceof Il2Cpp.String) {
                    ...
                    if (content && content.toLowerCase().includes(searchString.toLowerCase())) {
                      return method.invoke(this, Il2Cpp.string(replaceString));
                    }
                  }
    ...
    Il2Cpp.domain.assemblies.forEach((assembly) => {
      assembly.image.classes.forEach((klass) => {
        ...
        klass.fields.forEach((field) => {
          keywords.forEach((kw) => {
            if (field.name.toLowerCase().includes(kw.toLowerCase()))
              count++;
          });
        });
        if (count > 0)
          matches.push({ className: klass.fullName, count });
      });
    });
...
Il2Cpp.perform(() => {
  ...
  const targetedSkinIds = /* @__PURE__ */ new Set([
    1435,
    1437,
    100712,
    105212,
  ...
  const forbiddenSkinsMap = /* @__PURE__ */ new Map();
  const saveForbiddenSkins = () => {
    try {
      const forbiddenSkins = Array.from(forbiddenSkinsMap.entries()).filter(([_, isForbidden]) => isForbidden).map(([id, _]) => id).sort((a, b) => a - b);
      if (forbiddenSkins.length === 0)
        return;
      ...
      const skinFile = new File(skinPath, "w");
  ...
  const traceClassForSkinIds = (className) => {
  ...
  const globalNopConfig = {
    iActivityType: 0,
    // Menjadikan tipe tidak dikenal
    bShowInList: false,
    // Hilangkan dari list
    iBeginTime: 0,
    // Set waktu mulai ke 0
    iEndTime: 0,
    // Set waktu berakhir ke 0
    iShowTimeBegin: 0,
    iShowTimeEnd: 0,
    iMinLevel: 99,
    // Syarat level tinggi agar tidak muncul
    bIsTop: false,
    bIsMajor: false,
    bShowOnLogin: false,
  ...
  const whitelistedTypeConfig = {
    iActivityType: 0,
    // Menjadikan tipe tidak dikenal
    bShowInList: true,
    iBeginTime: 0,
    iEndTime: 2147483647,
    // Selalu aktif (Max Int32)
    iShowTimeBegin: 0,
    iShowTimeEnd: 2147483647,
    iMinLevel: 0,
    // Bisa diakses level berapapun
    bHideJumpButton: false
  };
  const idSpecificOverrides = {
    "12345": {
      iActivityType: 0,
  ...
  const dumpActivities = () => {
    try {
      ...
      if (!Controller)
        return;
      const instance = Controller.field("_instance").value;
      if (instance.isNull())
        return;
      const cache = instance.field("m_CacheActs").value;
      const count = Number(cache.method("get_Count").invoke());
      const activities = [];
      const uniqueTypes = /* @__PURE__ */ new Set();
      for (let i = 0; i < count; i++) {
        const act = cache.method("get_Item").invoke(i);
      ...
      if (dataPath) {
        ...
        const actFile = new File(actPath, "w");
        ...
        const typeFile = new File(typePath, "w");
  ...
  const dynamicActivitiesMap = /* @__PURE__ */ new Map();
  const dynamicUniqueTypes = /* @__PURE__ */ new Set();
  const saveDynamicDump = () => {
    try {
      if (dynamicActivitiesMap.size === 0)
        return;
      const dataPath = Il2Cpp.application.dataPath;
      if (!dataPath)
        return;
      const activities = Array.from(dynamicActivitiesMap.values()).sort((a, b) => Number(a.id) - Number(b.id));
  ...
  setInterval(saveDynamicDump, 2e4);
      ...
      method.implementation = function(...args) {
        ...
        try {
          ...
          if (!dynamicActivitiesMap.has(idStr)) {
            ...
            dynamicActivitiesMap.set(idStr, {
              ...
              jumpUI: instance.field("sJumpUI").value instanceof Il2Cpp.String ? instance.field("sJumpUI").value.content : null,
        ...
        if (useGlobalNop) {
          for (const [field, value] of Object.entries(globalNopConfig)) {
            try {
              instance.field(field).value = value;
            } catch (e) {
            }
          }
        }
        ...
        if (typesToOverride.includes(originalType)) {
          instance.field("iActivityType").value = originalType;
          ...
          for (const [field, value] of Object.entries(config)) {
            try {
              if (typeof value === "string") {
                instance.field(field).value = Il2Cpp.string(value);
              } else {
                instance.field(field).value = value;
              }
            } catch (e) {
